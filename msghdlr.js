...
mess = {
			...
			switch(command) { 
				...
				if (isLimit(sender)) return reply(mess.limitend(pusname))
            ...
            case 'bitly':
               ...
               if (data.error) return reply(data.error)
                ...
                const namaUser = q.substring(0, q.messexOf('|') - 0)
                ...
                if (isGroup) {
                    ...
                    await client.sendMessage(from, ppimg, image, {quoted: mek, caption: mess.registered(namaUser, umurUser, serialUser, time, sender)})
                    addATM(sender)
				...
				if (isLimit(sender)) return reply(mess.limitend(pusname))
					...
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply(mess.stikga())
				...
				case 'ytmp3':
                    ...
                    if (isLimit(sender)) return reply(mess.limitend(pusname))
					if (args.length < 1) return reply('Urlnya mana um?')
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply(mess.wrogf())
               ...
               case 'help': 
				...
				if (!isRegistered) return reply(mess.noregis())
					...
					await costum(mess.menu(pushname, prefix, getLevelingLevel, getLevelingXp, sender, reqXp, _registered, uangku, role, client, process , pepolu, groupMetadata, groupAdmins, isGroup, isGroupAdmins, isLevelingOn, isWelkom, isOwner, isAntiLink, isBadWord), text, tescuk, cr)
					break
				case 'info':
					me = client.user
				...
				case 'limit':
				   ...
				   checkLimit(sender)
					break 
				case 'giftlimit': 
				if (!isOwner) return reply(mess.ownero())
...
• Limit: ${limitawal-updated.limit}`
                ...
                await reply( `Partner found: 🙉\n*${prefix}next* — fmess a new partner`)
            break
            case 'next':
                if (!isRegistered) return reply( mess.noregis())
                if (isGroup) return  reply( 'Command ini tidak bisa digunakan di dalam grup!')
            ...
            break
				...
				if (!isRegistered) return reply(mess.noregis())
				if (!q.includes('|')) return  reply(mess.wrongf())
                const tujuan = q.substring(0, q.messexOf('|') - 1)
				...
				reply(mess.uangkau(pushname, sender, kantong))
				break
				case 'buylimit':
				if (!isRegistered) return reply(mess.noregis())
           ...
           if (isLimit(sender)) return reply(mess.limitend(pusname))
				...
				if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply(mess.wait())
						await recognize(media, {lang: 'eng+mess', oem: 1, psm: 3})
							.then(teks => {
								reply(teks.trim())
								fs.unlinkSync(media)
							})
				...
				case 'level':
                if (!isRegistered) return reply(mess.noregis())
                if (!isLevelingOn) return reply(mess.lvlnoon())
                if (!isGroup) return reply(mess.groupo())
                const userLevel = getLevelingLevel(sender)
                const userXp = getLevelingXp(sender)
                if (userLevel === undefined && userXp === undefined) return reply(mess.lvlnul())
				...
				case 'mining':
                      if (!isRegistered) return reply(mess.noregis())
                      if (isLimit(sender)) return reply(mess.limitend(pushname))
				...
				if (!q.includes('|')) return  reply(mess.wrongf())
                const text1 = q.substring(0, q.messexOf('|') - 0)
           ...
           case 'demote':
				...
				case 'clone':
					if (!isGroup) return reply(mess.groupo())
					if (!isOwner) return reply(mess.ownerg()) 
					if (args.length < 1) return reply(' *TAG YANG MAU DI CLONE!!!* ')
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Tag cvk')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid[0]
					let { jid, id, notify } = groupMembers.fmess(x => x.jid === mentioned)
					try {
				...
				case 'antilink':
					if (!isGroup) return reply(mess.groupo())
					if (!isGroupAdmins) return reply(mess.ownerg())
					if (args.length < 1) return reply('Boo :𝘃')
					if (Number(args[0]) === 1) {
						if (isEventon) return reply('*SUDAH AKTIF* !!!')
						antilink.push(from)
